...
/* BNS v440 - users/PIN-login fix: filter alleen orders, nooit users. old_ blijft archief. */
(function(){
  if (window.__BNS_V440_USERS_LOGIN_FIX__) return;
  window.__BNS_V440_USERS_LOGIN_FIX__ = true;

  function isDriverPhone(){
    var href = String(location.href || '').toLowerCase();
    var body = String(document.body && document.body.innerText || '').toLowerCase();
    return href.indexOf('driver') >= 0 ||
           href.indexOf('bezorger') >= 0 ||
           body.indexOf('bezorger tapwagen') >= 0 ||
           body.indexOf('mobiele opdrachten') >= 0;
  }

  function folderOf(o){
    return String((o && o.folder) || '').toLowerCase().trim();
  }

  // Deze functie mag door firebase-sync worden aangeroepen.
  // Belangrijk: users altijd teruggeven, anders werkt PIN-login niet.
  window.BNS_filterRowsForClient = function(col, rows){
    if (!Array.isArray(rows)) return rows;

    if (col === 'users') return rows;
    if (col !== 'orders') return rows;

    // Alleen de telefoon/bezorger hoeft streng alleen lopend te krijgen.
    if (!isDriverPhone()) return rows;

    return rows.filter(function(o){
      return folderOf(o) === 'lopend';
    });
  };

  window.BNS_filterDocForClient = function(col, row){
    if (!row) return row;

    if (col === 'users') return row;
    if (col !== 'orders') return row;

    if (!isDriverPhone()) return row;
    return folderOf(row) === 'lopend' ? row : null;
  };

  console.info('[BNS v440] users/PIN-login fix actief; old_ blijft archief; telefoon orders alleen lopend.');
})();
