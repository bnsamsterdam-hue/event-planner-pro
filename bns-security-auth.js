...
(function(){
  ...
  var VERSION = "BNS 540 Tapwagen login zonder Firebase teksten";
  ...
  function ensureStyle(){
    ...
    s.textContent =
      ...
      "#bnsSecurityPass{-webkit-text-security:disc;text-security:disc}"+
  ...
  function showOverlay(auth, authMod){
    ...
    div.innerHTML =
      '<form id="bnsSecurityAuthCard" autocomplete="off" data-lpignore="true" data-1p-ignore="true">'+
        '<h2>Tapwagen.nl toegang</h2>'+
        '<p>Log in met je Tapwagen.nl account. Daarna opent de normale app-login.</p>'+
        '<label>E-mail</label>'+
        '<input id="bnsSecurityEmail" type="text" inputmode="email" autocomplete="off" autocapitalize="none" autocorrect="off" spellcheck="false" name="tapwagen_gebruiker" data-lpignore="true" data-1p-ignore="true" value="'+esc(remembered)+'" placeholder="info@tapwagen.nl" required>'+
        '<label>Wachtwoord</label>'+
        '<input id="bnsSecurityPass" type="text" autocomplete="off" autocapitalize="none" autocorrect="off" spellcheck="false" name="tapwagen_toegang" data-lpignore="true" data-1p-ignore="true" placeholder="Wachtwoord" required>'+
        '<button id="bnsSecurityBtn" type="submit">Inloggen</button>'+
        '<div id="bnsSecurityAuthError"></div>'+
        '<small id="bnsSecurityAuthSmall">Beveiligde Tapwagen.nl toegang. De app-login blijft daarna voor Admin/Planner/Bezorger rechten.</small>'+
      '</form>';
    ...
    form.addEventListener("submit", async function(e){
      ...
      try{
        ...
        var msg = "Inloggen mislukt. Controleer je e-mail, wachtwoord of internetverbinding.";
        if(code === "auth/network-request-failed"){
          msg = "Inloggen mislukt. Controleer de internetverbinding.";
        }
        err.textContent = msg;
        try{ console.warn("[Tapwagen login] technische foutcode:", code); }catch(_){ }
  ...
  function installLogout(auth, authMod, user){
    try{
      ...
      b.type="button";
      b.title=(user&&user.email)?("Tapwagen.nl account: "+user.email):"Tapwagen.nl account";
      b.textContent="Account";
      b.onclick=async function(){
        if(!confirm("Uitloggen bij Tapwagen.nl? De app sluit dan af tot opnieuw inloggen.")) return;
  ...
  console.info("[BNS 540] Tapwagen.nl beveiligde toegang actief.");
})();
