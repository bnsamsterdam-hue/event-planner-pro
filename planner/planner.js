...
const BNS = {
  ...
  db: null,
  user: null,
  state: {users:[], orders:[], materials:[], customers:[], locations:[], alerts:[], settings:{}}
};
...
function toast(text){ const el=$("toast"); el.textContent=String(text||""); el.classList.add("show"); clearTimeout(el._timer); el._timer=setTimeout(()=>el.classList.remove("show"),3500); }
function hasRight(key){ return !!(BNS.user && BNS.user.rights && BNS.user.rights[key]); }
...
function addressOf(o){ const parts=[]; const add=v=>{v=clean(v); if(v&&!parts.includes(v))parts.push(v)}; [o.locationName,o.locationAddress,o.locationStreet,o.locationZip,o.locationCity,o.address,o.street,o.zip,o.city].forEach(add); if(o.location&&typeof o.location==="object"){[o.location.name,o.location.address,o.location.street,o.location.zip,o.location.city].forEach(add)} return parts.join(", "); }
...
function materialText(o){ const mats=o.materials||o.mats||[]; return Array.isArray(mats) ? mats.map(m=>typeof m==="string"?m:(m.code||m.name||"")).filter(Boolean).join(", ") : ""; }
function routeUrl(type, address){ const q=encodeURIComponent(address||""); return type==="waze" ? `https://waze.com/ul?q=${q}&navigate=yes` : `https://www.google.com/maps/search/?api=1&query=${q}`; }
...
async function loadCollection(name){
  const snap = await BNS.firebase.getDocs(BNS.firebase.collection(BNS.db, name));
...
async function updateOrder(order){
  ...
  await BNS.firebase.setDoc(BNS.firebase.doc(BNS.db, "orders", String(order.id)), order, {merge:true});
}

async function addAlert(alert){
  const id = alert.id || ("a_" + Math.random().toString(36).slice(2,10));
  alert.id = id;
  await BNS.firebase.setDoc(BNS.firebase.doc(BNS.db, "alerts", id), alert, {merge:true});
}

function populateUsers(filterFn){
  const users = (BNS.state.users || []).filter(filterFn);
  $("loginName").innerHTML = users.map(u=>`<option value="${esc(u.id)}">${esc(u.name)} (${esc(u.role || "Medewerker")})</option>`).join("") || `<option value="">Geen gebruikers gevonden</option>`;
}

function loginWithFilter(filterFn, sessionKey, afterLogin){
  ...
  if(!filterFn(found)){ toast("Geen rechten voor deze portal"); return; }
  BNS.user = found;
...
function restoreSession(filterFn, sessionKey, afterLogin){
  const id = sessionStorage.getItem(sessionKey);
  if(!id) return;
  const found = (BNS.state.users || []).find(u => String(u.id) === String(id));
  if(found && filterFn(found)){ BNS.user = found; afterLogin(); }
}
...
function getOrders(){
  return (BNS.state.orders || []).filter(o=>!isDeleted(o)).sort((a,b)=>dateTime(orderStart(a))-dateTime(orderStart(b)));
}

function card(o){
  const start=orderStart(o), end=orderEnd(o);
...
async function boot(){
  try{
    ...
    $("loginPin").addEventListener("keydown", e => { if(e.key==="Enter") loginWithFilter(plannerAllowed, SESSION_KEY, showApp); });
    $("logoutBtn").onclick = () => { sessionStorage.removeItem(SESSION_KEY); location.reload(); };
    $("refreshBtn").onclick = async () => { await loadAll(); render(); };
    $("searchBox").oninput = () => {
      const q=lower($("searchBox").value);
      document.querySelectorAll(".order").forEach(el=>el.style.display=!q||lower(el.innerText).includes(q)?"":"none");
    };
