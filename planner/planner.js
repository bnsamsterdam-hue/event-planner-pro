...
function plannerAllowed(u){const r=lower(u.role);return r==="planner"||r==="admin"||!!(u.rights&&(u.rights.orders||u.rights.admin||u.rights.planner))}
function getOrders(){return(BNS.state.orders||[]).filter(o=>!isDeleted(o)).sort((a,b)=>dateTime(orderStart(a))-dateTime(orderStart(b)))}
function orderCard(o){const s=orderStart(o),e=orderEnd(o),dl=s&&e&&s!==e?`${niceDate(s)} t/m ${niceDate(e)}`:niceDate(s||e);return `<article class="card order" data-id="${esc(o.id)}"><div class="order-title">${esc(o.number||"")} - ${esc(o.title||"Zonder titel")}</div><div class="rights"><span class="badge">${esc(o.status||"Open")}</span><span class="badge">Bezorger: ${esc(driverName(o)||"niet gekoppeld")}</span></div><div class="meta"><div>📅 <strong>${esc(dl)}</strong></div><div>👤 ${esc(customerName(o)||"Klant onbekend")}</div><div>🚚 ${esc(driverName(o)||"Geen bezorger")}</div><div>📍 ${esc(addressOf(o)||"Adres onbekend")}</div><div>📦 ${esc(materialText(o)||"Geen materialen")}</div></div><div class="row"><button class="btn btn-green" type="button" data-status="${esc(o.id)}" data-value="Uitgevoerd">Uitgevoerd</button><button class="btn btn-orange" type="button" data-status="${esc(o.id)}" data-value="Geannuleerd">Geannuleerd</button><button class="btn btn-dark btn-full" type="button" data-assign="${esc(o.id)}">Bezorger koppelen</button></div></article>`}
function showApp(){$("loginBox").classList.add("hidden");$("appBox").classList.remove("hidden");$("who").textContent=BNS.user?`${BNS.user.name} - ${BNS.user.role||"Planner"}`:"";render()}
function render(){const rows=getOrders();$("orders").innerHTML=rows.length?rows.map(orderCard).join(""):`<div class="empty">Geen opdrachten.</div>`;bindActions()}
function findOrder(id){return(BNS.state.orders||[]).find(o=>String(o.id)===String(id))}
function driverUsers(){return(BNS.state.users||[]).filter(u=>lower(u.role)==="bezorger"||!!(u.rights&&u.rights.gps))}
function bindActions(){document.querySelectorAll("[data-status]").forEach(b=>{b.onclick=async()=>{const o=findOrder(b.dataset.status);if(!o)return;if(!confirm(`Status wijzigen naar ${b.dataset.value}?`))return;o.status=b.dataset.value;o.updatedAt=new Date().toISOString();o.updatedBy=BNS.user.name||"";await updateOrder(o);toast("Status opgeslagen");await loadAll();render()}});document.querySelectorAll("[data-assign]").forEach(b=>{b.onclick=async()=>{const o=findOrder(b.dataset.assign);if(!o)return;const drivers=driverUsers();if(!drivers.length){toast("Geen bezorgers gevonden in Firebase");return}const list=drivers.map((d,i)=>`${i+1}. ${d.name}`).join("\\n");const answer=prompt("Kies bezorger nummer:\\n"+list,"1");const idx=Number(answer)-1;if(!drivers[idx])return;o.driverId=drivers[idx].id;o.bezorgerId=drivers[idx].id;o.driverName=drivers[idx].name;o.driver=drivers[idx].name;o.bezorger=drivers[idx].name;o.updatedAt=new Date().toISOString();await updateOrder(o);toast("Bezorger gekoppeld");await loadAll();render()}})}
async function boot(){try{await initFirebase();await loadAll();populateUsers(plannerAllowed);$("loginBtn").onclick=()=>loginWithFilter(plannerAllowed,SESSION_KEY,showApp);$("loginPin").addEventListener("keydown",e=>{if(e.key==="Enter")loginWithFilter(plannerAllowed,SESSION_KEY,showApp)});$("logoutBtn").onclick=()=>{sessionStorage.removeItem(SESSION_KEY);location.reload()};$("refreshBtn").onclick=async()=>{await loadAll();render();toast("Verversd")};$("searchBox").oninput=()=>{const q=lower($("searchBox").value);document.querySelectorAll(".order").forEach(el=>{el.style.display=!q||lower(el.innerText).includes(q)?"":"none"})};restoreSession(plannerAllowed,SESSION_KEY,showApp)}catch(e){console.error(e);setStatus("Fout: "+e.message)}}
boot();
